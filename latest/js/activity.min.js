...
angular.module("modules/activity/templates/item.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/item.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>道具管理</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\">关键字：</label><input type=\"text\" ng-model=\"key\" class=\"form-control mc-text-width\" placeholder=\"请输入道具名称或者商家名称\"></li><li><button ng-click=\"search()\" class=\"btn btn-primary ml10\" type=\"submit\"><i class=\"fa fa-search\"></i> 查找</button> <button ng-click=\"delete()\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 批量删除</button> <button ng-click=\"edit()\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 新增道具</button></li></ul></div></div></div></div><table class=\"table table-hover\"><thead><tr><th><input type=\"checkbox\" ng-model=\"selected\" ng-change=\"selectAll()\" value=\"\">全选</th><th>序号</th><th>名称</th><th>金额(元)</th><th width=\"30%\">商家名称</th><th>操作</th></tr></thead><tbody><tr class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\"><input type=\"checkbox\" ng-model=\"item.selected\"></td><td class=\"ng-binding\">{{$index+1}}</td><td class=\"ng-binding\">{{item.name}}}</td><td class=\"ng-binding\">{{item.money}}</td><td class=\"ng-binding\">{{item.businessName}}</td><td class=\"ng-binding\"><a ng-click=\"edit(item)\">编辑</a> <i class=\"text-divider\">|</i> <a ng-click=\"delete(item)\">删除</a></td></tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);
...
angular.module("modules/activity/templates/ticket.edit.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/ticket.edit.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">新增卡券</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券类型：</label><label class=\"radio-inline\"><input type=\"radio\" name=\"inlineRadioOptions\" ng-model=\"entity.type\" value=\"10\">返利券</label></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券名称：</label><input type=\"text\" ng-model=\"entity.name\" class=\"form-control mc-auto-width\" value=\"请输入卡券名称\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>金额：</label><input type=\"text\" ng-model=\"entity.value\" class=\"form-control mc-auto-width\" value=\"请输入卡券金额\"> <span class=\"corred\">注：当选择【再来一次券】时，金额非必填</span></li><li><label class=\"text-normal text-nowrap\">返利有效期：</label><input type=\"text\" ng-model=\"id\" class=\"form-control mc-auto-width\" ng-model=\"entity.eff\" value=\"请输入返利使用有效期\"><span>天</span><span style=\"font-size:12px;color:#999;margin-left:10px\">若不填，则时间不做限制</span></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券图片：</label></li></ul></div><div class=\"modal-footer ng-scope\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div>");
}]);
