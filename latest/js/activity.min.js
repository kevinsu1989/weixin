...
app.value('menubar', [{
    ...
    "menus": [{
      ...
      "href": "/web/menus/activity#activity/awardlist/-1",
      "name": "中奖管理",
      "id": 8000200002
    },{
...
activityModule.controller('acteditController', ['$scope', '$state', '$stateParams', '$modal', 'activityService', 'msgService',
	function($scope, $state, $stateParams, $modal, service, msgService) {
			...
			$modal.open({
				...
				controller: ['$scope', function(scope) {
					...
					scope.confirm = function() {
						...
						scope.entity.limitEndAt = (scope.entity.limitEndAt) ? service.getEndTime(scope.entity.limitEndAt) * 1 : '';
		...
		$scope.save = function() {
			...
			$scope.activity.rules[0].exPrizeDrawEndAt = ($scope.activity.rules[0].exPrizeDrawEndAt) ? service.getEndTime($scope.activity.rules[0].exPrizeDrawEndAt * 1) : '';
			$scope.activity.endAt = ($scope.activity.endAt) ? service.getEndTime($scope.activity.endAt * 1) : '';
...
//中奖管理
activityModule.controller('awardlistController', ['$scope', '$state', '$stateParams','activityService', 'selectService', 'msgService',
	function($scope, $state,$stateParams, service, selectService, msgService) {
...
activityModule.factory('activityService', function($http) {
    ...
    getEndTime:function(time){
      ...
      return new Date(date.getFullYear()+"/"+(date.getMonth()+1)+"/"+(date.getDate()+1))*1-1;
    }
	}
});
...
angular.module("modules/activity/templates/activity.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/activity.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>活动列表</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\">活动名称：</label><input type=\"text\" ng-model=\"id\" class=\"form-control mc-auto-width\" placeholder=\"请输入活动名称\"></li><li><button class=\"btn btn-primary ml10\" ng-click=\"search()\"><i class=\"fa fa-search\"></i> 查找</button> <button ui-sref=\"activity.actedit({id: -1})\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 创建活动</button></li></ul></div></div></div></div><table class=\"table table-hover\"><thead><tr><th>序号</th><th>活动名称</th><th>创建时间</th><th>活动开始时间</th><th>活动结束时间</th><th>中奖管理</th><th>状态</th><th class=\"text-right\">操作</th></tr></thead><tbody><tr ng-if=\"list.length!=0\" class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\">{{$index+1}}</td><td class=\"ng-binding\"><a ui-sref=\"activity.actedit({id:item.id})\">{{item.activityName}}</a></td><td class=\"ng-binding\">{{item.createAt|date:'yyyy-MM-dd HH:mm'}}</td><td class=\"ng-binding\">{{item.startAt|date:'yyyy-MM-dd HH:mm'}}</td><td class=\"ng-binding\">{{item.endAt|date:'yyyy-MM-dd HH:mm'}}</td><td class=\"ng-binding\"><a ui-sref=\"activity.awardlist({id:item.id})\">中奖管理</a></td><td class=\"ng-binding\">{{item.status|activityStatus}}</td><td class=\"text-right\"><a ng-click=\"update(item)\">{{item.status|actStatus}}</a></td></tr><tr ng-if=\"list.length==0\">没有数据</tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);
...
angular.module("modules/activity/templates/ticket.edit.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/ticket.edit.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">新增卡券</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券类型：</label><label class=\"radio-inline\"><input style=\"margin-top:13px\" type=\"radio\" name=\"inlineRadioOptions\" ng-model=\"entity.type\" value=\"10\">返利券</label></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券名称：</label><input type=\"text\" ng-model=\"entity.name\" class=\"form-control mc-auto-width\" value=\"请输入卡券名称\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>金额：</label><input type=\"text\" ng-model=\"entity.value\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" class=\"form-control mc-auto-width\" value=\"请输入卡券金额\"></li><li><label class=\"text-normal text-nowrap\">返利有效期：</label><input type=\"text\" ng-model=\"id\" class=\"form-control mc-auto-width\" ng-model=\"entity.eff\" value=\"请输入返利使用有效期\"><span>天</span><span style=\"font-size:12px;color:#999;margin-left:10px\">若不填，则时间不做限制</span></li><li><label>卡券图片：</label><button ng-if=\"!entity.logoUrl\" class=\"btn btn-primary\" ng-click=\"updateImg()\" ng-disabled=\"processing\">添加图片</button> <img ng-if=\"entity.logoUrl\" height=\"50\" ng-click=\"updateImg()\" ng-src=\"{{entity.logoUrl|imgFilter}}\" id=\"__logoImg__\"> <img ng-if=\"!entity.logoUrl\" height=\"50\" id=\"__logoImg__\"></li></ul></div><div class=\"modal-footer ng-scope\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div><form style=\"display:none\" action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" target=\"_uploadIFrame\"><input type=\"file\" name=\"file\" id=\"imgFile\"> <input type=\"submit\" value=\"upload\" id=\"imgSubmit\"></form><iframe name=\"_uploadIFrame\" style=\"display:none\"></iframe>");
}]);
